...
(() => {
  ...
  // src/lib/utils.ts
  var clampNumber = (num2, a, b) => Math.max(Math.min(num2, Math.max(a, b)), Math.min(a, b));
  ...
  var transformColor = (target, seed, reference) => {
    ...
    const c4 = target.c * scaleC;
    const h = seed.h;
    return formatHex(oklch({ l, c: c4, h }));
  };
